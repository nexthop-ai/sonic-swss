...
bool OrchDaemon::init()
{
    ...
    */
    ...
    else if (platform == BRCM_PLATFORM_SUBSTRING)
    {
        ...
        // Setting PFC_DLR_INIT_ENABLE to 0 asks for a software handler
        // explicitly, so it also keeps the hardware watchdog out of the way. The
        // virtual switch advertises every queue attribute, DLDR included, so
        // without this the software handlers become unreachable there and the
        // tests that set this variable can no longer exercise them.
        bool forceSoftwareWd = false;
        ...
        if (gSwitchOrch->checkPfcDldrEnable() && !forceSoftwareWd)
        {
